...
function OutLineItem(props) {
  ...
  return (
    <div className={`${style.container} ${currentPage === pageNumb ? style.active : ''}`} onClick={slidePage}>
      <div className={style.content}>
        <img ref={$img} src={errImg} className={style.img} />
        {loading && (
          <div id="outline-loading-item" className={style.loading}>
            <svg className={style.icon} id="icon-loading" viewBox="0 0 1024 1024"><path d="M164.592 786.848l56.56 56.56 147.088-147.072-56.576-56.576-147.072 147.088z m678.816-565.696l-56.56-56.56-147.088 147.072 56.56 56.576 147.088-147.088z m-678.816 0l147.088 147.088 56.56-56.56-147.088-147.088-56.56 56.56zM272 464H64v80h208v-80z m367.76 232.32l147.088 147.088 56.56-56.56-147.072-147.088-56.576 56.56zM464 944h80V736h-80v208z m272-480v80h208v-80H736zM464 272h80V64h-80v208z" fill="#565D64"></path></svg>
          </div>
        )}
      </div>
      <p className={style.pageNumb}>
        <span className={style.pageTag}>{pageNumb + 1}</span>
      </p>
    </div>
  );
}
