...
function __mockPenToMouse(data) {
  ...
  const {
    onPenDraw = true, drawAppLeft = stage._getContentPosition().left, drawAppTop = stage._getContentPosition().top, drawAppScrollTop = whiteBoardInstance.el.parentElement.scrollTop,
    drawAppWidth = stage.width(),
    drawAppHeight = stage.width() * STAND_DOC_RATIO,
  ...
  const clientX = drawAppLeft + drawAppWidth * nX;
  const clientY = drawAppTop + drawAppHeight * nY;
  const dataX = STAND_WIDH * nX;
  ...
  const dataY = c_top * STAND_WIDH / drawAppWidth + STAND_HEIGHT * nY;
