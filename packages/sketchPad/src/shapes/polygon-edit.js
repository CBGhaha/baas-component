...
export class PolygonEdit extends Shape {
  constructor(stage, style, clientid, toolname, showtype, data) {
    ...
    if (style && style.bgImage) {
      const points = data[1];
      this.line.fillPatternImage(style.bgImage);
      console.log('theImgsss:', style, showtype, data);
      const topPonitX = points[0];
      const topPonitY = points[1];
      const shapeWidth = points[4] - topPonitX;

      const scale = shapeWidth / style.bgImage.width;
      console.log('scalesss:', shapeWidth, this.line.fillPatternX(), this.line);
      this.line.fillPatternScale({ x: scale, y: scale });
      this.line.fillPatternOffsetX(-topPonitX / scale);
      this.line.fillPatternOffsetY(-topPonitY / scale);
      this.line.fillPatternRepeat('no-repeat');
    } else {
