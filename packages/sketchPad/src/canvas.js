...
import { testdata, reBuildTextStroke, getActionItemUid, isMobile, loadImages } from './util';
...
export class Canvas {
    ...
    if (this.cacheDataInstance.currentLayerId !== id) {
      ...
      if (cacheData) {
        for (let i = 0; i < cacheData.length; i++) {
          ...
          await this.replayDataItem(item, serverRatio, 'replayData');
        }
  ...
  async pushDataToLayer(data = [], layerid = null) {
    const len = data.length;
    if (len > 0) {
      const serverRatio = this.scaletoserver;
      if (
  ...
  async replayDataItem(item, serverRatio, replykind) {
    ...
    if (existItem) {
      ...
      console.log('ShapeClass1:');
      if (bgImage) {
        bgImage = await loadImages(bgImage);
        console.log('bgImage:', bgImage);
      }
      const shape = new ShapeClass(
        this.stage,
        isFill ?
          { stroke: bgImage ? null : stroke, fill: stroke, bgImage } :
          {
            stroke: bgImage ? null : stroke,
      ...
      if (item[item.length - 1] === true || shape.toolname === DRAWTOOL_TEXTTOOL) {
        ...
        shape.draw();
        shape.endDraw((shape.toolname === DRAWTOOL_SMOOTHLINE ? item[4] : null), item[3] === DRAWTOOL_POLYGON_EDIT && item[8]);
      } else {
