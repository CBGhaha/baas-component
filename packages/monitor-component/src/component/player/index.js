...
import useEventController from '../common/hooks/useEventController';
import DrawTools from '../common/components/drawTools/index';
...
import { calZmlScope } from '../common/utils/index';
...
export default function Room(props) {
  ...
  const [coursewareId, setCoursewareId] = useState(eventControllersInstance.$store.getState().initCourseware.id);
  const [scope, setScope] = useState({});
  const [pageNum, setPageNum] = useState(eventControllersInstance.$store.getState().initCourseware.page);
  ...
  const { userInfo } = eventControllersInstance.$store.getState();
