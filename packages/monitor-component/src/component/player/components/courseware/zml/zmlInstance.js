...
export default class ZmInstance {
  ...
  constructor(url, handleMessage, eventControllersInstance) {
    ...
    this.eventControllersInstance.controllers.whiteBoardController.on('zmlMessage', (payload)=>{
      ...
      const { userInfo: { role } } = this.eventControllersInstance.$store.getState();
      // 处理zml的答题消息
      if (action === 'questionOperation') {
        const isDoAnswer = operation && operation.doAnswer;
        if (role === ZM_USER_TYPE.teacher && zmlRole !== 'student') {
          this.sendIsAnsweringToQt(!!isDoAnswer, payload.data);
        } else if (role === ZM_USER_TYPE.tutor) {
          payload.data = {};
        }
        this.eventControllersInstance.$store.dispatch(commonAction('isZmlExaming', !!isDoAnswer));
      }

      // 处理zml的内容滚动

      if (action === 'scrollTop' && role !== ZM_USER_TYPE.teacher) {
  ...
  // 初始化
  setZmlInitialize() {
    const { userInfo } = this.eventControllersInstance.$store.getState();
  ...
  async setZmlUserGroup(bool) {
    ...
    const { userInfo: { role }, studentsMap } = this.eventControllersInstance.$store.getState();
    if (role !== ZM_USER_TYPE.tutor) {
      if (studentsMap) {
  ...
  handlerIframeMsg = async ({ data }) => {
    ...
    if (action === 'sendInteractionArea') {
      ...
      this.eventControllersInstance.$store.dispatch(commonAction('zmlCoverEleList', areas.map(item=> ({ ...item, handleClick: (data)=>{
