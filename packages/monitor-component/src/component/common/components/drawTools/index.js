...
function DrawTool(props) {
  ...
  const isNotHold = isAudience(eventControllersInstance.$store);
  ...
  useEffect(() => {
    // 初始化教具实例
    drawToolsInstance.current = dawToolsInstance(document.getElementById('draw'), ZM_USER_TYPE.teacher, signalType, eventControllersInstance);
