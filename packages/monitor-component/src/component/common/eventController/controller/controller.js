...
export default class Controller extends EventEmitter {
  ...
  async handleEventMsg(msg) {
    ...
    if (action) {
      console.log('%c components- receive:', 'color:#00FFFF', name, data);
  ...
  emitSocket(event, ... arg) {
    console.log('%c components-request:', 'color:#FFD700', event, arg);
    return new Promise((resolve) => {
      this.sendEventHandle(event, arg, (res)=>{
        console.log('%c components-response:', 'color:#8A2BE2', event, res);
        resolve(res);
      });
    });
  }
